<?php
$servername = "Connection Information go here";
$username = "Connection Information go here";
$password = "Connection Information go here";
$dbname = "Connection Information go here";
$bookID = $_GET['bookID'];

try {
    $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
    // set the PDO error mode to exception
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    // sql to delete a record
    $sql = "DELETE FROM bookClub WHERE bookID= :bookID";

    // use exec() because no results are returned
    $cmd = $conn->prepare($sql);
    $cmd->bindParam(':bookID', $bookID, PDO::PARAM_INT);
    $cmd->execute();
    echo 'Your record info was successfully added to the database. <br/> <a href="recordForBook.php"> View Updated Book Club Info</a>';
    }
catch(PDOException $e)
    {
    echo $sql . "<br>" . $e->getMessage();
    }

$conn = null;
?>
